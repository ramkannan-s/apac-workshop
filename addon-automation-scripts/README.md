...
- [APAC Workshop 2023 - Lab Setup Guide](#apac-workshop-2023---lab-setup-guide)
  ...
  - [Master Scripts to Setup Lab](#master-scripts-to-setup-lab)
  - [Individual Scripts to Trigger (If the `master` script is not used)](#individual-scripts-to-trigger-if-the-master-script-is-not-used)
...
- Install [pwgen](https://lindevs.com/install-pwgen-on-ubuntu) in Ubuntu or use `brew install pwgen` for Mac. 
...
## Master Scripts to Setup Lab

* Perform `chmod +x apac-workshop-main.sh` inside the folder. 

* Update values.json present in the folder `secret/values.json` with all the required data :-

* Trigger the script :-
```bash
./apac-workshop-main.sh
```

* The above script is a single shot script to perform all the below actvities. 

* If the data upload is required then set this boolean `use_transfer_data_bool` to **true**. 


## Individual Scripts to Trigger (If the `master` script is not used)
